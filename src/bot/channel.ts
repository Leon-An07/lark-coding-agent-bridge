...
async function intakeMessage(deps: IntakeDeps): Promise<void> {
  ...
  // Push the possibly-threadId-backfilled message (emsg) so the batched flush
  // reads the recovered threadId for reply routing and topic scope.
  ...
  try {
    ...
    if (replyMode === 'card') {
      ...
      const streamDone = channel.stream(
        chatId,
        {
          markdown: async (ctrl) => {
            producerStarted = true;
            markdownCtrl = ctrl;
            await ctrl.setContent(markdownReply(latestState));
            await renderDone;
          },
        },
        sendOpts,
      ...
      await awaitRenderAwareStream({
        ...
        producerStarted: () => producerStarted,
        fallback: async (state) => {
          const body = markdownReply(state);
          if (body.trim()) {
