...
describe('locale independence of the bridge system prompt', () => {
  afterEach(() => {
    setActiveLocale(DEFAULT_LOCALE);
  });

  it('instructs replying in the language the user writes in', () => {
    expect(BRIDGE_SYSTEM_PROMPT).toContain('回复语言跟随用户');
    expect(BRIDGE_SYSTEM_PROMPT).toContain('始终使用用户消息所用的语言回复');
  });

  it('is NOT affected by the bridge UI locale — the agent follows the user, not the bridge', () => {
    // Bridge i18n covers bridge-owned surfaces only (cards, command replies,
    // CLI). The system prompt stays a single variant so the bridge language
    // preference cannot steer the agent's reply language.
    const zh = buildBridgeSystemPrompt(undefined);
    for (const locale of ['en-US', 'ja-JP'] as const) {
      setActiveLocale(locale);
      expect(buildBridgeSystemPrompt(undefined)).toBe(zh);
      const withIdentity = buildBridgeSystemPrompt({ openId: 'ou_bot_self', name: 'Nemo' });
      expect(withIdentity).toContain('## 你的身份');
      expect(withIdentity).toContain('ou_bot_self');
      const wrapped = prefixBridgeSystemPrompt('hello world', undefined);
      expect(wrapped.startsWith(zh)).toBe(true);
