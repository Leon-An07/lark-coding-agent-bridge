...
| `/mention group off` | 当前群进入专属群免 @ 模式（普通消息也触发，不建议多 bot 群开启） |
| `/mention group default` | 当前群跟随 `/config` 的全局 @ 设置 |
...
私聊不需要 @。群和话题群默认必须 `@bot`；owner / admin 可在目标群里用 `/mention group off` 把该群设为“专属群免 @”（普通消息也触发），用 `/mention group on` 单独恢复必须 @，用 `/mention group default` 恢复跟随 `/config` 全局设置。免 @ 只适合这个群基本只给当前 bot 使用的场景；多 bot 群建议保持必须 @，否则普通消息可能让多个 bot 同时响应。`@all` 会被忽略。支持的云文档评论里 @bot 就会触发回复。
...
- 改完**下一条消息**就生效，不用重启。
- **群里默认要先 @bot 才会回**（私聊不用 @）。这是另一个独立开关（`/config` →"群里需要 @ bot"），和上面的名单是两回事。免 @ 更像“专属群模式”，多 bot 群不建议开。
...
`allowedUsers` / `admins` 填用户 `open_id`，`allowedChats` 和 `requireMentionInGroupOverrides` 的 key 填群 `chat_id`；override 的 `false` 表示该群是专属群免 @ 模式，`true` 表示该群必须 @bot。手动找 ID 最简单的办法：让对方给 bot 发条消息（群里就 @ 它一下），然后看当前 profile 的日志：
